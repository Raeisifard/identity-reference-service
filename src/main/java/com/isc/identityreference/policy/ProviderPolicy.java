...
public record ProviderPolicy(
        ...
        Set<ProviderClaim> supportedClaims,
        Map<IdentityField, FieldOwnership> fieldOwnership,
        Duration ttl,
        Duration staleGrace,
        List<RefreshWindow> refreshWindows,
        Duration maxJitter,
        RequestQuota quota,
    ...
    public ProviderPolicy {
        ...
        requireNonNegative(staleGrace, "staleGrace");
        ...
        Objects.requireNonNull(photoPolicy, "photoPolicy");
        Objects.requireNonNull(embeddingPolicy, "embeddingPolicy");
    }

    public boolean supports(ProviderClaim claim) {
        return supportedClaims.contains(Objects.requireNonNull(claim, "claim"));
    }

    public FieldOwnership ownershipOf(IdentityField field) {
        return fieldOwnership.getOrDefault(Objects.requireNonNull(field, "field"), FieldOwnership.UNSPECIFIED);
    }
    ...
    public record RequestQuota(long maxRequestsPerSecond, int maxConcurrentRequests) {
        public RequestQuota {
            ...
            if (maxConcurrentRequests <= 0) throw new IllegalArgumentException("maxConcurrentRequests must be positive");
        }
    }
    ...
    public record EmbeddingPolicy(boolean enabled, String modelId, String modelVersion, int dimension) {
