...
public final class OracleIdentityReferenceStore implements IdentityReferenceStore {
    ...
    public OracleIdentityReferenceStore(IdentityReferenceRepository repository){this.repository=repository;}
    ...
    public IdentityReference save(IdentityReference r){return save(r,r.freshness().freshUntil().minus(Duration.ofHours(6)),1L);}
    public IdentityReference save(IdentityReference r,Instant nextRefreshAt,long policyVersion){
        var e=repository.findByLookupKeyHash(LookupKeyFingerprint.of(r.lookupKey())).orElseGet(IdentityReferenceEntity::newEntity);
        e.setId(r.id().value().toString()); e.setLookupKeyHash(LookupKeyFingerprint.of(r.lookupKey()));
        e.setGivenName(r.attributes().givenName()); e.setFamilyName(r.attributes().familyName()); e.setFatherName(r.attributes().fatherName());
        e.setBirthDate(r.attributes().birthDate()); e.setGender(r.attributes().gender()); e.setNationalIdCiphertext(r.attributes().nationalId().getBytes(StandardCharsets.UTF_8));
        e.setLifecycleState(r.lifecycleState().name()); e.setAcquiredAt(r.freshness().acquiredAt()); e.setFreshUntil(r.freshness().freshUntil()); e.setStaleUntil(r.freshness().staleUntil()); e.setNextRefreshAt(nextRefreshAt);
        ...
        e.setPolicyVersion(policyVersion); e.setRefreshStatus("SUCCESS"); e.setRefreshError(null); e.setRefreshRetryCount(0); e.setCreatedAt(r.createdAt()); e.setUpdatedAt(r.updatedAt());
        repository.save(e); return r;
    }
    public int currentRefreshRetryCount(IdentityLookupKey key){return repository.findByLookupKeyHash(LookupKeyFingerprint.of(key)).map(e->e.getRefreshRetryCount()==null?0:e.getRefreshRetryCount()).orElse(0);}
    public void recordRefreshFailure(IdentityLookupKey key,String providerId,String status,String safeError,int retryCount,Instant nextRefreshAt,Instant updatedAt){
        repository.findByLookupKeyHash(LookupKeyFingerprint.of(key)).ifPresent(e->{
            e.setProviderId(providerId); e.setRefreshStatus(status); e.setRefreshError(safeError==null?null:safeError.substring(0,Math.min(256,safeError.length())));
            e.setRefreshRetryCount(retryCount); e.setNextRefreshAt(nextRefreshAt); e.setUpdatedAt(updatedAt); repository.save(e);
        });
    }
